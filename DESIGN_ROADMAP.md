...
- [x] **C1 🟠 Pannelli ridisegnati.** Card con shadow stratificati, padding 24–32px, border sottile→hover, h3 con micro-label uppercase mono. *DoD: pannelli "Contenuto/Design/Anteprima" cinematografici.*
- [x] **C2 🟠 Form di livello.** Input height 48px, focus ring `rgba(accent,.12)`, label sopra (già ok), color-picker con swatch arrotondato e anteprima live. Validazione inline. *DoD: form che rispetta la sezione FORM DESIGN.*
- [x] **C3 🟠 Stage anteprima QR cinematografico.** Cornice del QR con sfondo a gradient mesh sottile, ombra morbida, transizione su update. Stato vuoto illustrato (non testo grigio). *DoD: anteprima che "vende".*
...
- [x] **D1 🟠 Product-card per i QR.** Griglia `auto-fill minmax(280px,1fr)`, thumb QR su superficie chiara, badge dyn/stat in mono, hover translateY + shadow-lg + border-accent. *DoD: lista QR come card-grid premium.*
- [x] **D2 🟠 Empty state illustrato.** SVG + messaggio empatico + CTA "Genera il tuo primo QR". *DoD: nessuna lista mostra testo grigio nudo.*
- [x] **D3 🟠 Skeleton loading.** Sostituire i "…" e i caricamenti con skeleton shimmer (card, analytics). MAI spinner. *DoD: fetch mostra skeleton.*
...
- [x] **D5 🟢 Modali e toast.** Modal con backdrop blur + entrata spring; toast con icona stato (ok/err), barra di durata. Il `confirmModal` già introdotto eredita lo stile. *DoD: feedback coerente.*
...
- [x] **F1 🔴 Verifica RLS su tutte le tabelle.** Confermare/abilitare policy su `tenants`, `memberships`, `branding`, `qr_codes`, `scans` (l'anon key è pubblica). *DoD: query cross-tenant negata.*
...
- [x] **F3 🟠 Stati di caricamento sui bottoni.** Save/Bulk/Dominio: bottone disabilitato + label "…in corso" durante le await (anti doppio-submit). *DoD: nessun doppio invio possibile.*
- [x] **F4 🟠 Validazione input runtime.** URL/email/telefono validati prima del save; WiFi/vCard con escape dei caratteri speciali (`;`, `,`, `:`). *DoD: payload QR sempre validi.*
- [x] **F5 🟢 Bulk robusto.** Progress bar reale (n/totale), report righe saltate, limite dimensione/righe, ZIP nominato con data. *DoD: bulk affidabile su CSV grandi.*
...
- [x] **F8 ⚪ Pulizia.** Rimuovere `STATE.editingDynamicId` inutilizzato, centralizzare `randCode`/`escapeHtml`, commenti solo dove serve. *DoD: zero dead code.*
